    ...
    <div class="w">
      ...
      <div class="cartbottom">
        ...
        <div class="settlement">
          <router-link to="/">
            <el-button type="primary">返回主页面</el-button>
          </router-link>
        </div>
...
export default {
  ...
  methods: {
    ...
    del(index) {
      ...
      let n = 0;
      const login = JSON.parse(localStorage.getItem('islogin'));
      const ordername = `${login.name}(${'order'})`;
      const data1 = JSON.parse(localStorage.getItem(ordername));
      ...
      const data3 = JSON.parse(localStorage.getItem('islogin'));
      const ordername1 = `${data3.name}(${'order'})`;
      localStorage.setItem(ordername1, JSON.stringify(data2));
    },
  },
};
</script>
